...
/**
 * プリビルドスクリプト: ランディングコンテンツのバンドル生成
 *
 * このスクリプトは、ビルド時に実行され、ランディングページのコンテンツを
 * TypeScriptモジュールにバンドルします。Cloudflare Workersでは
...
async function generateLandingContent() {
  try {
    console.log('🚀 Starting landing content generation...');
    ...
    // 1. content/blog/landing配下のターゲットディレクトリを読み込む
    const contents = {};
    const mangaAssets = {};
    ...
    try {
      ...
      console.log('   ⚠️ Landing directory does not exist, creating empty bundle');
      targets = [];
    }
    ...
    for (const target of targets) {
      ...
      try {
        ...
        console.log(`   ✅ Loaded content: ${target}`);
      } catch (error) {
        console.error(`   ❌ Failed to load content ${contentYamlPath}:`, error.message);
        continue;
      }

      // manga/ ディレクトリの画像一覧を取得
      ...
      try {
        ...
        mangaAssets[target] = [];
        console.log(`   ⚠️ No manga directory for: ${target}`);
      }
    }

    console.log(`📝 Found ${Object.keys(contents).length} landing targets`);
...
/**
 * ランディングページコンテンツ（ビルド時生成）
 *
 * このファイルは、ビルド時にcontent/blog/landing/配下のYAMLファイルから
 * 自動生成されます。Cloudflare Workersではファイルシステムアクセスができないため、
...
`;
    ...
    console.log('\n✨ Landing content generation completed successfully!');
    console.log(`\n📊 Summary:`);
