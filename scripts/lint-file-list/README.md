...
❌ file-list.mdに未定義のファイルが3件検出されました:
  ...
  📄 app/lib/service-name/implementation-flow/unexpectedLogic.ts
  📄 app/data-io/service-name/implementation-flow/unexpectedApi.server.ts

対処方法:
  1. 不要なファイルの場合 → 削除してください
  2. 必要なファイルの場合 → file-list.mdに追加してください

file-list.md: develop/service-name/implementation-flow/file-list.md
```
...
ℹ️  file-list.mdに定義されているが実在しないファイルが2件あります:
  ...
  📄 app/lib/service-name/implementation-flow/plannedLogic.ts
```
...
1. **共通ファイルを別のfile-list.mdに分離**: `develop/service-name/common/file-list.md`を作成し、共通ファイルを定義
...
運用開始後、以下の条件が確認された場合、UI統合（service-nameへの統合）を検討します。

- lintコマンドの実行忘れが頻発
- service-nameのUIで一元管理したいという強い要望
- リアルタイム可視化の必要性が確認される

UI統合時は、`scripts/lint-file-list/lib/*.js`を`app/lib/service-name/`へ移植し、Remixの`loader`から呼び出す形で実装します。
