...
- **Before**: Lighthouseで「使用していないJavaScript: 39 KiB」「使用していないCSS: 21.8 KiB」という警告が表示。CSS最適化を優先し、JavaScript削減は後回しに。
- **Current**: CSS最適化は成功（21.8 KiB → 11.8 KiB、46%削減）。しかし、JavaScript削減は失敗（39 KiBのまま）。lazy loading、manual chunk splitting、prefetch無効化など複数の手法を試したが、効果なし。
- **Next**: Remix/Viteの共有バンドル戦略という根本原因を理解し、JavaScript削減を諦めることを決断。他の最適化領域（画像、フォント、キャッシュ等）に注力する方針に転換。

## 具体的なタスク

- **Before**:
  Lighthouseレポートで「使用していないJavaScript: 39 KiB」の警告を確認。CSS最適化が成功したため、同じ手法でJavaScript削減も可能だと考えた。

- **Current**:
  lazy loadingでReact.lazyを導入、manual chunk splittingでルートごとにバンドル分割、prefetch="none"でナビゲーションリンクのプリフェッチを無効化。しかし、39 KiBという数字は変わらず。

- **Next**:
  Remix/Viteの共有バンドル戦略（複数ルートで共通のコンポーネントを効率的にバンドルする仕組み）が原因と判明。フレームワークの制約を受け入れ、JavaScript削減を諦め、他の最適化領域に注力する。
