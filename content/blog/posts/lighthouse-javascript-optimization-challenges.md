...
### フレームワークの制約を無視した最適化の代償

- フレームワークの設計思想を無視した最適化を続けることで、コードベースには「設計の歪み」が蓄積されていきます。
- 無理やり実装した最適化がフレームワークのアップデートで破綻し、大規模なリファクタリングを強いられることになる。
- 保守性が失われたコードは技術的負債となり、新機能の追加もバグ修正も困難になり、プロジェクトが停滞する。

### 制約を理解した上での戦略的撤退

- この記事を読めば、Lighthouse最適化の限界点を見極め、フレームワークの制約を理解する視点が手に入ります。
- lazy loading、chunk splitting、prefetch制御という3つの手法の検証結果と、失敗の分析プロセスを確認できます。
- 本ブログ自身で検証し、39 KiBのJavaScriptが削減できなかった理由を実証した記録を学べます。
- AIと協業しながら最適化の限界を探り、現実的な落とし所を見つけるための知見が手に入ります。
