...
- `waitForTimeout`は麻薬であり、一度許容するとあらゆる場所に伝染し、クリック後に1秒、ページ遷移後に1秒といった根拠のない待機が蔓延する。
- 100テストケースで300秒以上の無駄な待機が発生し、テスト実行のたびに5分以上のロスが生じ、開発効率が著しく低下する。
- 開発サイクルが遅くなり、「テストを回すのが面倒」という空気がチームに蔓延し、やがてテストへの信頼そのものが崩壊する始まりとなる。

### こんな未来が手に入ります

- `waitForTimeout`をプロジェクトルールで禁止すべき理由と、代替となる「状態ベース待機」のパターンが手に入る。
- CLAUDE.mdに追記すべき具体的なルールと、Playwrightが提供する適切な待機メソッドの一覧を習得できる。
- この方法は机上の空論ではなく、まさにこのブログ自身のE2Eテストで実際に遭遇し、ルール化した実証済みの知見である。
- AIとの協調開発において「何を禁じるべきか」を定義する、プロジェクト設計の本質的な情報を習得できる。
