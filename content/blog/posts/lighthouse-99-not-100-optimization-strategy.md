...
- **Before**:
  Lighthouseで94点、LCP 2.4s。外部フォントが750ms、CSSが470ms×2でブロック、JSバンドルに45%の未使用コード。

- **Current**:
  フォントセルフホスト化 (-750ms)、CSS preload削除 (-1,000ms)、JSバンドル分割 (-19 kB)、クリティカルCSS最小インライン化。99点達成、LCP 1.5s。

- **Next**:
  100点を追わない。CSSフルインライン化は初回-200ms だが2ページ目+300ms。font-display: optional + preload は効果限定的でリスク高。
