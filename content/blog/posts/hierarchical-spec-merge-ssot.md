...
- 仕様、実装、テストの定義がバラバラになり、情報の整合性が個人の記憶という極めて不安定な基盤に委ねられます。
- ラベル一つ変えただけで「なぜか壊れたE2Eテスト」の調査に数時間が溶け、開発の勢いが確実に削ぎ落とされていきます。
- 修正の影響範囲を誰も把握できなくなり、やがてシステムは「下手に触ると何が起こるか分からない」不気味なブラックボックスへと変貌します。
