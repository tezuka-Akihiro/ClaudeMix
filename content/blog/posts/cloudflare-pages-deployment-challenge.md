...
プロジェクト全体を調査したところ、**ブログ記事のMarkdownファイルを読み込む処理など、20以上のファイルでファイルシステムを直接読み書きするコードが使われている**ことが判明しました。
...
User Request → SSR → モジュール参照 → HTML
```

このアプローチを実現するため、以下の3つの戦術を実行しました：

1. **ビルド前処理の導入**: コンテンツファイルを読み込んで型安全なモジュールを自動生成
2. **データアクセス層の書き換え**: ファイルシステムへの依存を完全排除し、生成されたモジュールを参照
3. **ビルドプロセスの統合**: CI/CD環境でも自動的にコンテンツが生成される仕組み
