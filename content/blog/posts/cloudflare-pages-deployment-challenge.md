...
最新のRemixアプリをCloudflare Pagesにデプロイしようとした。
しかし、ローカル環境では問題なく動くのに、デプロイすると「ファイルシステムがない」といったエラーが出てしまい、途方に暮れてしまった。
なんとか回避策を見つけても、パフォーマンスが落ちたり、機能が制限されたりして、根本的な解決にはならなかった。
...
ローカル環境でのみ動作確認を続け、デプロイ環境の制約を軽視することで、あなたのアプリには「見えないデプロイリスク」が静かに蓄積されていきます。
やがて、本番環境で予期せぬエラーが発生し、ユーザーがアクセスできない状態に陥るでしょう。
ついに、ユーザーはエラーに耐えきれず離脱し、あなたのアプリは「動かない」という評判だけが残り、誰も使わない"デジタル廃墟"と化すかもしれません。

### こんな未来が手に入ります

この記事を読めば、Cloudflare Pagesの制約を逆手に取り、パフォーマンスを向上させる設計思想が手に入ります。
具体的には、ファイルシステムに依存しない「ビルド時バンドル」というアプローチで、RemixアプリをCloudflare Pagesに安定してデプロイする**設計図**を手に入れられます。
この方法は、机上の空論ではありません。まさに**このブログ自身のアーキテクチャとして実証済み**です。
この情報は、単なるデプロイ手順ではなく、サーバーレス環境の制約を「最適化の機会」に変える**未来の開発現場から得られた一次情報**です。

### 私も同じでした

筆者も過去に同じデプロイ問題で悩み抜き、このブログを「AIによる実装」と「人間による設計」で作り上げる過程でこの解決策を見つけました。
この記事で、Cloudflare Pagesの制約を理解し、安定したデプロイを実現するための基本的な考え方と、明日から試せるTipsを持ち帰れるように書きました。
さらに深掘りして、サーバーレス環境に最適化されたアーキテクチャ設計を知りたい方は、その詳細な実装方法を確認できます。

## 📝 概要

Remix v2 + Viteで構築したブログアプリをCloudflare Pagesにデプロイする過程で、CSSインポートエラーから始まり、最終的にCloudflare環境の**ファイルシステム（※）非互換**という大きな壁に突き当たりました。

この記事では、それらの問題を特定し、解決策として**「ビルド時にコンテンツをバンドル（※）する」**アプローチを選択し、実装するまでの全プロセスを詳細に記録します。

> ※ **ファイルシステム**: コンピューターがファイル（例: `my-document.txt`）を保存したり読み込んだりする仕組み。
...
ローカル開発環境では問題なく動作するブログアプリが、Cloudflare Pagesにデプロイするとサーバーエラーで表示されなくなりました。
...
デプロイエラーの根本原因にたどり着くまでに、CSSのビルドエラーやSSR（※）設定の不備といったいくつかの問題を段階的に解決しました。

> ※ **SSR (サーバーサイドレンダリング)**: ユーザーがページをリクエストするたびに、サーバー側でページを生成して返す仕組み。

しかし、これらの問題を修正しても、今度は`Could not resolve "fs"`のような、Node.js（※）環境に特有のAPIが見つからないというエラーに直面しました。

> ※ **Node.js**: サーバーサイドでJavaScriptを動かすための環境。多くの開発ツールで利用されています。

プロジェクト全体を調査したところ、**ブログ記事のMarkdownファイルを読み込む処理など、20以上のファイルでファイルシステムを直接読み書きするコード（`fs/promises`）が使われている**ことが判明しました。

```bash
# プロジェクト内で 'fs/promises' を使っているファイルを検索
$ grep -r "fs/promises" app/ --include="*.ts" --include="*.tsx"
# 結果: 20以上のファイルで 'fs/promises' を使用
```
...
調査の結果、**Cloudflare Workers/Pagesの実行環境では、サーバーのファイルシステムに一切アクセスできない**という制約が根本原因であると特定しました。

ローカル開発環境では自由にファイルを読み込めますが、サーバーレス環境ではその「常識」が通用しなかったのです。
...
User Request → SSR → fs.readFile(".../post.md") → HTML

After (ビルド時にファイルを埋め込む):
Build Time: ".../post.md" → JavaScriptモジュールに変換
User Request → SSR → import(module) → HTML
```

このアプローチを実現するため、ビルド前にMarkdownファイルを読み込んでTypeScriptモジュールを生成するスクリプトを作成しました。
...
そして、`package.json`のビルドコマンドを修正し、`remix vite:build`の前にこのスクリプトが実行されるようにしました。
