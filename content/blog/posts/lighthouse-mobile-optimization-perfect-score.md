...
特に、CSS設計思想（STYLING_CHARTER.md）を壊さないよう注意しました。例えば、CSSインライン化は一時的なパフォーマンス向上にはなりますが、Layer構造（1-5層）の分離が崩壊し、保守性が低下するため採用しませんでした。

### ぶつかった壁

最も大きな壁は、**アーキテクチャの一貫性を保ちながら最適化を進める**ことでした。

例えば、Lighthouseは「CSSをインライン化してレンダリングブロックを削減せよ」と提案しますが、このブログは厳格なCSS階層アーキテクチャ（globals.css → layer2.css → layer3.ts）を採用しています。安易にCSSをHTMLに埋め込むと、この設計思想が崩壊してしまいます。
...
例えば、CSSインライン化は一時的なパフォーマンス向上には有効ですが、このブログの厳格なCSS階層設計（STYLING_CHARTER.md）を壊してしまいます。Layer 1（globals.css）→ Layer 2（layer2.css）→ Layer 3（layer3.ts）という分離を維持することで、長期的な保守性を確保できます。
