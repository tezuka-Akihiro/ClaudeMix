...
各エージェントは自分の「目先の正解」を押し付けてくる。Stripeエージェントには「Payment Element方式」が正解。プロジェクトの設計書には「Checkout方式」が正解。どちらも「正しい」。しかし両立はしない。
やがて、AIの実装を「まあ動いてるし」と受け入れ続けると、設計書とコードの乖離が広がっていく。
ついに、誰も全体像を把握できなくなり、どの変更が何を壊すか分からなくなる。そして100時間のデバッグ地獄が始まる。

### AIを黙らせる「SSoT違反」という武器

この記事を読めば、AIが出してきた「動いているっぽい何か」に自信を持ってダメ出しできるようになります。
具体的には、「SSoT違反」という概念でAIを黙らせ、汚染されたコードを見抜く**設計図**を手に入れられます。
この方法は、まさにこのブログの開発中に発生したインシデントから得られた実践知です。
この情報は、公式ドキュメントには書かれていない、エージェント乱立時代の生存戦略です。
