# 記事企画書: ライブラリを捨ててWeb標準へ回帰する —— Cloudflare Workersで「10年死なない」認証基盤を築くための外科的切断
...
**何をしたか（What）**:
51ファイルに及ぶライブラリ依存のコードを「外科的切断（削除）」し、Web標準（Request/Response/Crypto）のみを用いた疎結合な認証ロジックへと再構築しました。
...
1. **既存ライブラリの外科的切断**: Remix Authに関連する51ファイルを削除。ライブラリによるブラックボックス化されたセッション管理と認証ロジックを排除しました。
...
2. **AIエージェントが陥った「増築の罠」**: 複雑なライブラリのエラーをAIに解決させようとした結果、不自然なパッチ当てが重なり、51ファイルもの「つぎはぎ」が発生。システムの生存性が著しく低下しました。
3. **セキュリティの自己責任化**: ライブラリに預けていたOAuthのState検証やセッションの隠蔽化を、自前で、かつ脆弱性なく実装し直すという高い技術的ハードルがありました。

---

### どう乗り越えたか（Solution）

1. **ブランチの削除とゼロベースの決断**: 51ファイルの修正を「延命医」として勇気を持って切り捨て、ライブラリを一切使わない最小構成から再出発しました。
...
Cloudflare Workersのようなエッジ環境で、Node.js前提のライブラリを使おうとすると、ローカルでは動くのにデプロイした瞬間に謎の「isSession」エラーで止まる。AIに修正させればさせるほどコードは肥大化し、気づけば51ファイルもの「つぎはぎ」が。この不自然な構造は、プロダクトの死を早める毒でしかありません。
...
AIエージェントが陥った罠から、51ファイルのコードを捨て去った決断、さらにWeb Crypto APIを用いた具体的な署名実装まで。あなたが「死なないための拠点」を築くための、泥臭くも確実な歩みを紹介します。
...
- **案1**: ライブラリを捨ててWeb標準へ回帰する —— Cloudflare Workersで「10年死なない」認証基盤を築くための外科的切断
- **案2**: 認証ライブラリがエッジを殺す：51ファイルのつぎはぎコードを捨てて得た「構造の正しさ」
- **案3**: Remix × Cloudflare Edgeにおける「認証の延命医」：ライブラリ依存から脱却しWeb標準APIで基盤を再定義する
