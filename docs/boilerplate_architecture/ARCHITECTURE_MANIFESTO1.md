...
| :--- | :--- | :--- |
| 🎨 **UI層** | `app/routes/` `app/components/` | **関心事**: ユーザーへの表示とインタラクション。 **テスト**: `loader`/`action`の統合テスト、コンポーネントのレンダリング・操作テスト。 |
| 🧠 **純粋ロジック層** | `app/lib/` | **関心事**: 副作用のない純粋なビジネスロジック（計算、データ変換等）。 **テスト**: 入力と出力のみを検証する純粋なユニットテスト。**テストカバレッジ100%**を目指す。 |
| 🔌 **副作用層** | `app/data-io/` | **関心事**: 外部世界との通信（DBアクセス、APIコール等）。 **テスト**: 外部通信をモック化した上での入出力テスト。 |
...
- **使い方**:
  ...
  - 複数のサービスで共有されるコンポーネント: `app/components/service-name/`
