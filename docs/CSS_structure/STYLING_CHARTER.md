...
- user-managementなど、どのサービスでも「どのような役割で使うか」を示すトークンを提供
...
**禁止される命名パターン**:
- ❌ **サービス名プレフィックス**: `.servicename-*`, `.user-management-*` などのサービス名をプレフィックスとして使用することは禁止
...
### 禁止される命名パターン
・**サービス名プレフィックス** (例: `.servicename-*`, `.user-management-*`)
