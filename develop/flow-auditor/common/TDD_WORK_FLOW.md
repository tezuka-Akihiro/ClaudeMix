...
**開発名**: Common Components の実装
**目的**: サービス全体のページレイアウトと、共有コンポーネント（ヘッダー、フッターなど）の設計・実装を管理する。
...
- **1. E2Eテストの準備**:
  - **`common`セクションの場合**: 画面レベルのE2Eテストファイル `tests/e2e/screen/flow-auditor.screen.spec.ts` を**新規作成**します。
...
**実装対象**:
1. **Layer 2**: `app/styles/flow-auditor/layer2.css`
2. **Layer 3**: `app/styles/flow-auditor/layer3.ts`
3. **Layer 4**: `app/styles/flow-auditor/layer4.ts`（必要な場合のみ）
...
- **1. UIコンポーネントの作成**:
  - **`common`セクションの場合**:
    - ページコンテナ（`app/routes/flow-auditor.tsx`）と、共有コンポーネント（`Header`, `Footer`など）を生成します。
    - **依頼例**: `@GeneratorOperator "flow-auditor サービスのページコンテナと共有コンポーネントを作成して"`
  - **`common`セクション以外の場合**:
    - 機能セクションのUIコンポーネント（例: `DesignFlowSection.tsx`）を生成します。
    - **依頼例**: `@GeneratorOperator "flow-auditor サービスの common セクションに、Header という名前のUIコンポーネントを作成して"`
...
- **1. ファイル生成**: `@GeneratorOperator` に依頼して、純粋ロジック層のファイルを生成します。
  - **依頼例**: `@GeneratorOperator "flow-auditor サービスの common セクションに、sectionListBuilder という名前のlibファイルを作成して"`
- **2. テスト実装 (RED)**: `sectionListBuilder.test.ts` に、正常系・異常系・境界値のテストケースを記述します。
- **3. 実装 (GREEN)**: `sectionListBuilder.ts` を実装し、テストをパスさせます。
- **4. リファクタリング**: ロジックをより効率的で読みやすい形に改善します。

#### 3.3. 🔌 副作用層の実装
- **1. ファイル生成**: `@GeneratorOperator` に依頼して、副作用層のファイルを生成します。
  - **依頼例**: `@GeneratorOperator "flow-auditor サービスの common セクションに、loadServiceList.server という名前のdata-ioファイルを作成して"`
- **2. テスト実装 (RED)**: `loadServiceList.server.test.ts` に、外部依存をモック化した上で、正常系と異常系のテストを記述します。
- **3. 実装 (GREEN)**: `loadServiceList.server.ts` を実装し、テストをパスさせます。
...
| 日付 | 作業内容 | 完了項目 | 次回予定 |
|------|----------|----------|----------|
| 2025-10-11 | Phase 1 & 2.1  | Phase 1: E2E (Happy Path) <br> Phase 2.1: UI (SectionSelector, Header, Footer) | Phase 2.2: 純粋ロジック層 |
| 2025-10-19 | ファイルリスト整備 | `file-list.md`, `retryTargetCalculator.ts`, `retryTargetValidator.ts` | TDD_WORK_FLOW.md更新 |

## 6. 学んだこと・気づき
- TDDプロセスにより、仕様の曖昧さが早期に発見できた
- 層分離により、テストの記述が容易になった

## 7. さらなる改善提案
- Toast通知コンポーネントの実装（右クリックパスコピー機能）
