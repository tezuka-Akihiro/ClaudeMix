...
graph TD
    ...
    C --> IX["LoadMoreButton (もっと見るボタン: 記事一覧の最下部)"]

    F --> J["CategorySelector (カテゴリセレクター: ドロップダウン)"]
    F --> K["TagGrid (タググリッド: グループ化対応)"]
    F --> L["FilterSubmitButton (決定ボタン)"]

    K --> K1["TagGroupContainer × N (各技術グループのコンテナ)"]
    K1 --> K2["TagGroupHeader (グループ見出し)"]
    K1 --> K3["TagGroupGrid (グループ内タググリッド)"]
    K3 --> K4["TagButton × N (タグボタン)"]
...
**責務**: 追加記事を読み込むボタンとして、ユーザーが記事一覧をスクロールして続きを読めるようにする

#### LoadMoreButton の状態

- **通常 (default)**: "More" ラベルのボタンが表示されている状態
- **ローディング (loading)**: スピナーアイコンと "Loading..." テキストが表示され、ボタンが無効化された状態
- **非表示 (hidden)**: 全件読み込み済みでボタンが非表示の状態

#### LoadMoreButton のインタラクション

1. **ボタンクリック**:
   - **トリガー**: LoadMoreButtonをクリック
   - **応答**: useFetcherを使用してサーバーから追加記事を取得
   - **パラメータ**: 現在の読み込み済み件数（loaded）とフィルタパラメータ（category, tags）を送信
   - **重要**: フィルタパラメータを保持すること
   - **ローディング中**: ボタンを無効化し、スピナーと "Loading..." テキストを表示
   - **取得完了**: 取得した記事を既存の記事一覧の最下部に追加表示

2. **全件読み込み済み**:
   - **条件**: loadedCount >= totalPosts の場合
   - **応答**: ボタンを非表示にする
...
- **キーボードナビゲーション**: Tab キーでフォーカス移動、Enter/Space でボタンクリック
...
- **PostCard**: ライトモード/ダークモードに応じて背景色、テキスト色、ボーダー色が自動的に切り替わります
- **FilterPanel**: モーダルの背景、ボーダー、シャドウがテーマに応じて変化します
- **Pagination**: ボタンとリンクの配色がテーマに応じて変化します
