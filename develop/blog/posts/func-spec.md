...
1. **記事一覧表示**: 投稿されたブログ記事を時系列で一覧表示
   - 各記事カードに以下の情報を表示:
     - **サムネイル画像**（lg/smバリアント、srcset対応、ゼロ設定方式）
     - 記事タイトル
     - 投稿日（YYYY-MM-DD形式）
     - カテゴリバッジ
     - タグバッジ（複数）
   ...
   - **サムネイル画像のCLS対策**:
     ...
     - サムネイルが存在しない場合は非表示（fallback: hide）
...
4. **カテゴリベースのアクセス制御**:
   ...
   - **認証状態の取得**: Routeが`getSession.server.ts`を呼び出し、セッションから`userId`を取得
...
- **記事一覧**: 現在のページに表示する記事の情報。各記事は以下を含む
  ...
  - カテゴリ
  - タグ配列
  - **サムネイルURL**（R2上のthumbnail.webpのURL、ゼロ設定方式で自動生成）
