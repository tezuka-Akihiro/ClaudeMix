...
**セクション定義** - このサービスは以下のセクションで構成されます：

- **`common`**: サービス全体で共有されるヘッダー、フッター、レイアウトコンポーネント、およびテーマ切り替え機能を提供
...
| Theme | テーマ | サイト全体の配色スキームを表す概念。ライトモードとダークモードの2種類を提供 |
| Light Mode | ライトモード | 明るい背景に暗いテキストを表示する配色テーマ。日中の明るい環境での可読性を重視 |
| Dark Mode | ダークモード | 暗い背景に明るいテキストを表示する配色テーマ。暗い環境での眼への負担を軽減 |
...
| Category-based Access Control | カテゴリベースのアクセス制御 | 記事のカテゴリに応じて、未ログインユーザーのアクセス可否を制御する仕組み。フリーミアムモデルの第1段階 |
| Freemium Model | フリーミアムモデル | 基本機能は無料、高度な機能は有料という段階的な価値提供モデル。未認証→ログイン→有料の3段階で構成 |
