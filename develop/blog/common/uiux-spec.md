...
| **BlogFooter** | フッターエリア。コピーライト表記のみを表示（簡素化版） |
...
**責務**: コピーライト表記を表示する（簡素化版）

#### BlogFooterの状態 (State)

- **通常 (default)**: コピーライト表記が表示されている状態

#### BlogFooterのインタラクション (Interaction)

- インタラクションなし（静的な表示のみ）

> **注意**: 法的リンク（利用規約、プライバシーポリシー、特定商取引法）とLegalModalは、ランディングページ固有の要件となったため、`LandingFooter`コンポーネントと`LegalModal`コンポーネント（`app/components/blog/landing/`）で実装されています。
...
| デバイス | 対応方針 |
| :--- | :--- |
| **モバイル (< 768px)** | NavigationMenuはモーダル形式で全画面表示。画像は `sm` バリアントを優先 |
| **タブレット (768px ~ 1024px)** | NavigationMenuはドロップダウン形式で表示。画像は `lg` バリアントを優先 |
| **デスクトップ (> 1024px)** | NavigationMenuはドロップダウン形式で表示。画像は `lg` バリアントを優先 |

### 画像配信戦略（レスポンシブ・バリアント）

パフォーマンス最適化のため、デバイスサイズに応じて最適な画像バリアントを配信します。

| 項目 | 仕様 |
| :--- | :--- |
| **境界（Breakpoint）** | 767px (mobile-max) |
| **lg バリアント** | 1200px (品質60, effort 9)。768px以上で使用 |
| **sm バリアント** | 600px (品質50, effort 9)。767px以下で使用 |
| **実装方法** | `srcset` および `sizes="(max-width: 767px) 600px, 1200px"` |
| **CLS防止** | `aspect-ratio: 1200 / 630` を全デバイスで固定 |
