...
1. **BlogFooter**: ブログ全体のフッター
   - コピーライト表示
   - 法的リンクの表示（利用規約、プライバシーポリシー、特定商取引法）
     - **利用規約**: `/blog/terms`へのリンク（ブログ記事として実装）
     - **プライバシーポリシー**: `/blog/privacy`へのリンク（ブログ記事として実装）
     - **特定商取引法**: モーダルトリガー（クリックでLegalModal表示）
   - 配置: `app/components/blog/common/BlogFooter.tsx`

2. **LegalModal**: 特定商取引法表示用モーダル
   - **目的**: 個人情報（自宅住所・本名）の検索エンジン露出を防止
   - **機能**:
     - モーダルオーバーレイ表示
     - 特定商取引法の内容を表示
     - 閉じるボタン（×ボタン）
     - Escキーで閉じる
     - モーダル外クリックで閉じる
     - モーダル表示時のスクロール制御（背景スクロール無効化）
   - **SEO対策**: モーダル内コンテンツは`noindex, nofollow`として扱う（検索エンジンから保護）
   - 配置: `app/components/blog/common/LegalModal.tsx`
...
- **フッターリンク**: フッターに表示する法的リンク集。各項目は表示ラベル、リンク先パス、モーダルフラグを持つ
  - 利用規約リンク（`/blog/terms`）
  - プライバシーポリシーリンク（`/blog/privacy`）
  - 特定商取引法（モーダルトリガー）
- **特定商取引法コンテンツ**: モーダルに表示する特定商取引法の内容（事業者情報、住所、連絡先等）
