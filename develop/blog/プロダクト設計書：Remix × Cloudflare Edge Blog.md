...
### 1.1. コンセプト
> 「Web標準への回帰と、エッジコンピューティングによるUI/UXの極地」

Next.js（ブラックボックスな魔法）へのアンチテーゼとして、Web本来の仕組み（HTTP/HTML）と最先端のインフラ（Edge）を組み合わせた、「理にかなった速さ」を実証する技術ブログ。

### 1.2. 目的 (KGI)
- **最大目的**: Remix × Cloudflare領域での**「第一人者候補」としてのポジショニング確立**。
- **到達目標**: 技術選定権限を持つCTOやリーダー層からのスカウト、または高単価案件の獲得。
- **手段**: 実際にアクセスして「速い」「快適」と感じさせ、ソースコードを見て「美しい」と納得させる。

### 1.3. ターゲットユーザー (ペルソナ)
- **Primary (採用権限者)**:
  - 技術的負債やNext.jsの複雑化に疲弊しているテックリードやCTO。
  - 「流行りだから」ではなく「合理的だから」技術を選ぶ組織。
- **Secondary (同調者)**:
  - Reactエコシステムの現状に疑問を持つ、感度の高いエンジニア。
...
- ダークモード対応: CSS変数の理解とシステム設定への追従（UX基本）。

### 3.2. 技術アピール機能 (Differentiators)
ここが勝負所です。「なぜNext.jsのSSGではなく、RemixのSSRなのか」を証明します。

- **リアルタイム・カウンター (D1活用)**:
  - 記事ごとの「閲覧数（PV）」と「いいね数」をSSR時に動的に取得して表示。
  - **意図**: 「静的サイト（SSG）ではありません、DBアクセスしてもこの速さです」という証明。
...
---
・次やること
この設計に基づいて、現在作成中の**「Remixボイラープレート」を、まずはこのブログ用に向けてカスタマイズしていく。
現段階は構想中なのでまだ進めない。