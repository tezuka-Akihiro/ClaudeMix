...
1. **サムネイル画像の表示**: R2にサムネイルが存在する場合、記事ヘッダーに表示（ゼロ設定方式）
   - **表示位置**: 記事ヘッダー部分（タイトルの上または下）
   - **レスポンシブ対応**: `srcset` を使用し、`lg` (1200px) と `sm` (600px) を提供。 `sizes="(max-width: 767px) 600px, 1200px"` を指定
   - **CLS対策**:
     - `loading="lazy"` 属性で遅延読み込み
     - `aspect-ratio: 1200 / 630` でレイアウトシフト防止
     - `decoding="async"` で非同期デコード
   - **フォールバック**: サムネイルが存在しない場合は非表示（fallback: hide）

2. **記事詳細の表示**: URLパラメータ（slug）から記事を特定し、詳細内容を表示する
...
- **サムネイル画像表示**: thumbnailUrlがある場合、記事ヘッダーにサムネイル画像を表示
  - CLS対策: aspect-ratio: 1200 / 630
  - 遅延読み込み: loading="lazy"
...
- **画像データ処理**: マークダウン内の画像記法（`![alt](path)`）を適切なHTMLの`<img>`タグに変換
  - **レスポンシブ対応**: 命名規則（`[n].lg.avif`）に基づき、自動的に `sm` バリアントの `srcset` を生成して付与
  - 遅延読み込み: `loading="lazy"` 属性を付与
  - スタイル設定: `style="max-width: 100%"` を設定
