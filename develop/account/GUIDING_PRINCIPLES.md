...
- **参照元**: `develop/blog/{section}/spec.yaml`
- **管理方針**: テストで使用する定数やメッセージテキストは`spec.yaml`を参照しますが、テストデータそのものは`spec.yaml`に定義しません。
- **テストデータの方針**:
  - サンプル記事データはマークダウンファイル形式で提供し、テストではこれを動的に読み込みます。
  - テストデータのハードコードは原則として避け、どうしても必要な場合に限り、その必要性を慎重に検討した上でテストスクリプト内に記述します。
