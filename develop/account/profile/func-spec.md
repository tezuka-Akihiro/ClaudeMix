...
1. 削除確認ダイアログ表示
2. パスワード検証
3. **アクティブなサブスクリプションの確認**（D1: subscriptionsテーブル）
4. **サブスクリプションが存在する場合、Stripeで即時解約**（`cancelStripeSubscription.server`）
5. **サブスクリプションレコード削除**（D1: subscriptionsテーブル）
6. ユーザーデータ削除（D1: usersテーブル）
