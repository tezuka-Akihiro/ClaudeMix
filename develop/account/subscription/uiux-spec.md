...
AccountLayout
  └─ Main Content Area
      ├─ SubscriptionStatusDisplay（契約中の場合）
      │   ├─ プラン名・価格
      │   ├─ 有効期限（自動更新ON/OFF明示）
      │   └─ 決済エラー時の通知（past_due時）
      │
      └─ PlanSelector（未契約またはプラン変更）
          ├─ プランカード
          └─ 自動更新中断中はプラン変更ボタンを disabled 化
```
...
- **自動更新ON時**:
    - ボタン：`[自動更新の中断に進む]`
    - 確認ダイアログ：「中断しても YYYY/MM/DD までは全機能を利用可能です。次回の更新のみを停止します」と明示。
- **自動更新OFF時**:
    - ボタン：`[自動更新を再開する]`
