...
test.describe('Account Subscription Management', () => {
  ...
  test.describe('Subscription Status - Inactive', () => {
    ...
    test('should display plan cards for inactive user', async ({ page }) => {
      ...
      const planCard = page.locator('[data-testid="plan-card-membership"]');
      await expect(planCard).toBeVisible();

      // Verify plan card content
      await expect(planCard.locator('.plan-name')).toContainText('メンバーシッププラン');
      await expect(planCard.locator('.plan-price__amount')).toContainText('¥500');
