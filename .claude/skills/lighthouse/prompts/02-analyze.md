...
Phase 1 のステップ 1.5 で取得した `psi-summary.mjs` の出力を分析し、コードベースの調査と合わせて原因を特定する。

**サマリーから確認すべきポイント**:

- `Metrics:` 行の ❌ マークが付いた指標（閾値超過）
- `LCP Element:` と `LCP Breakdown:` からLCPボトルネックの特定
- `Issues:` の各項目の `LCP+Xms` 推定削減量
- `Resources:` のリソースサイズ分布

**サマリーが利用できない場合**（手動測定フォールバック時）: `AskUserQuestion` でオペレータに追加情報を依頼する:
