# 【機能追加】blog記事のサブスクリプション状態に応じたアクセス制御

- **サービス**: `blog`
- **セクション**: `post-detail`
- **関連ドキュメント**:
  - `develop/blog/post-detail/func-spec.md`
  - `develop/blog/post-detail/uiux-spec.md`
  - `develop/blog/post-detail/file-list.md`
  - `develop/blog/post-detail/data-flow-diagram.md`
  - `develop/account/subscription/TDD_WORK_FLOW.md` (依存先)

---

## 1. 提案概要

note型の「導入部分は公開、本編は会員限定」という表示制御を実現し、サブスクリプション状態に応じて記事の可視範囲を動的に制御することで、コンテンツの収益化を支援します。

## 2. 変更内容 (As-Is / To-Be)

### 現状 (As-Is)

- すべてのブログ記事は全文が無料で閲覧可能
- サブスクリプション機能とブログ機能の連携が存在しない
- コンテンツの収益化機能がない

### 修正後 (To-Be)

- 記事のfrontmatterで`freeContentPercentage`（公開割合、0-100%）を指定可能
- 未契約ユーザーには指定割合までのコンテンツのみ表示し、それ以降にペイウォール（続きを読むための障壁）を表示
- ペイウォール内に購読促進バナーを配置し、`/account/subscription`へのCTAボタンで会員登録を促す
- 有効なサブスクリプションを持つユーザーには、記事全文を制限なく表示

## 3. 背景・目的

### 背景

- `project.toml`にてpost-detailセクションの`abstract_purpose`に「サブスクリプション状態に応じたアクセス制御を行う」と定義されているが、実際の設計ドキュメントには反映されていない
- accountサービスのサブスクリプション機能が実装予定であり、その活用先としてblog記事のアクセス制御が必要

### 目的

- **目的1**: note型のペイウォールモデルを実現し、コンテンツの収益化を支援
- **目的2**: accountサービスとblogサービスの連携を確立し、サービス間のシナジーを創出
- **目的3**: ユーザーにサブスクリプション価値を明確に提示し、会員登録を促進

## 4. 変更の妥当性 (Pros / Cons)

Pros (利点):

- サービス間連携の実現: blogサービスからaccountサービスのdata-io層を呼び出す明確なパターンを確立
- 責務の適切な分離: サブスクリプション状態の「管理」はaccountが担当し、「状態を元に記事を部分表示する」のはblogが担当
- ビジネス価値の向上: コンテンツの収益化モデルを提供
- 3大層アーキテクチャの遵守: アクセス制御判定をlib層（純粋関数）として実装し、テスト容易性を確保

Cons (懸念点):

- サービス間依存の発生: blogサービスがaccountサービスのdata-io層に直接依存
- 実装の複雑化: セッション管理、サブスクリプション状態取得、可視範囲判定など多層のロジックが必要
- accountサービスの実装が先行して必要: `getSubscriptionByUserId.server.ts`が実装されていることが前提

**総合評価**:

Consは存在するものの、プロジェクトの目標である「Remix MVPボイラープレート」として、サービス間連携のベストプラクティスを示すという点で、この変更は**非常に妥当性が高い**と判断します。また、責務の分離が適切に行われており、3大層アーキテクチャの原則に合致しています。

## 5. 設計フロー

以下の設計ドキュメントを上から順に確認し、編集内容を追記しました。

### 🗾GUIDING_PRINCIPLES.md

**実施済み**（変更なし: post-detailセクションはGUIDING_PRINCIPLESに影響しない）

### 📚️func-spec.md

**実施済み**

追加内容:
- 基本機能6: サブスクリプション状態に応じたアクセス制御
- Route責務: サブスクリプション状態取得の処理フロー追加
- Component責務: ペイウォール・購読促進バナー表示追加
- 純粋ロジック要件: `determineContentVisibility.ts`（コンテンツ可視範囲判定）追加
- 副作用要件: `getSubscriptionStatus.server.ts`（サブスクリプション状態取得）追加

### 🖼️uiux-spec.md

**実施済み**

追加内容:
- データフロー設計: Loaderにサブスクリプション状態取得処理追加
- レイアウト構造: Paywall Container、Subscription Promotion Banner追加
- インタラクションと状態遷移: Paywall、SubscriptionPromotionBannerの状態とインタラクション定義

### 📋️spec.yaml

**未実施**（現時点では不要と判断）

理由: サブスクリプション関連の設定値は`app/specs/account/subscription-spec.yaml`を参照するため、post-detailセクションのspec.yamlには追加不要

### 🗂️file-list.md

**実施済み**

追加ファイル:
- UI層: `Paywall.tsx`, `Paywall.test.tsx`, `SubscriptionPromotionBanner.tsx`, `SubscriptionPromotionBanner.test.tsx`
- 純粋ロジック層: `determineContentVisibility.ts`, `determineContentVisibility.test.ts`
- 副作用層: `getSubscriptionStatus.server.ts`, `getSubscriptionStatus.server.test.ts`
- 既存ファイルの更新: `PostDetailSection.tsx`（サブスクリプション状態に基づくコンテンツ制御追加）

### 🧬data-flow-diagram.md

**実施済み**

追加内容:
- Mermaid図: 「サブスクリプション状態取得フロー」「アクセス制御判定フロー」追加
- Route層責務: サブスクリプション状態取得、アクセス制御判定追加
- Component層責務: Paywall、SubscriptionPromotionBanner追加
- Pure Logic層責務: `determineContentVisibility.ts`追加
- Data-IO層責務: `getSubscriptionStatus.server.ts`追加
- データフローの説明: サブスクリプション関連のフロー追加

## 6. TDD_WORK_FLOW.md 簡易版

以下の全項目に対して、実際のパスと編集内容を1行で記載します。特に、accountサービスとの連携に注意が必要です。

### 👁️e2e-screen-test

**パス**: `tests/e2e/screen/blog/blog-post-detail.spec.ts`

**編集内容**: サブスクリプション状態に応じたアクセス制御のE2Eテスト追加（未契約ユーザーはペイウォール表示、契約ユーザーは全文表示）

### 👁️e2e-section-test

**パス**: `tests/e2e/section/blog/post-detail.spec.ts`

**編集内容**: 既存テストに加えて、ペイウォール表示、購読促進バナーのCTAクリック、可視範囲制御のテスト追加

### 🎨CSS実装 (layer2.css, layer3.ts, layer4.ts)

**パス**:
- `app/styles/blog/layer2-post-detail.css`
- `app/styles/blog/layer3.ts`

**編集内容**:
- Layer 2: `.paywall`, `.subscription-promotion-banner`コンポーネントクラス追加
- Layer 3: ペイウォールと購読促進バナーのレイアウト構造追加（必要に応じて）

### 🪨route

**パス**: `app/routes/blog.$slug.tsx`

**編集内容**:
- loaderにサブスクリプション状態取得処理追加（`getSubscriptionStatus.server.ts`呼び出し）
- `determineContentVisibility.ts`を呼び出してアクセス制御判定
- loaderの返り値に`showFullContent`、`visiblePercentage`、`hasActiveSubscription`追加

**重要な依存関係**: accountサービスのsessionデータ取得が必要（`getSessionBySessionId.server.ts`）

### 🚧components.test

**新規ファイル**:
- `app/components/blog/post-detail/Paywall.test.tsx`: ペイウォールコンポーネントのテスト
- `app/components/blog/post-detail/SubscriptionPromotionBanner.test.tsx`: 購読促進バナーのテスト

**既存ファイル更新**:
- `app/components/blog/post-detail/PostDetailSection.test.tsx`: サブスクリプション状態に応じたコンテンツ制御のテスト追加

### 🪨components

**新規ファイル**:
- `app/components/blog/post-detail/Paywall.tsx`: ペイウォールコンポーネント（SubscriptionPromotionBannerを内包）
- `app/components/blog/post-detail/SubscriptionPromotionBanner.tsx`: 購読促進バナー（プラン情報表示、CTAボタン）

**既存ファイル更新**:
- `app/components/blog/post-detail/PostDetailSection.tsx`: `showFullContent`に基づいてPaywall表示制御追加

### 🚧logic.test

**新規ファイル**:
- `app/lib/blog/post-detail/determineContentVisibility.test.ts`: アクセス制御判定ロジックのテスト（純粋関数）

**テストケース**:
- サブスクリプションアクティブ → 全文表示
- サブスクリプション非アクティブ → 部分表示
- 境界値（0%/100%）

### 🪨logic

**新規ファイル**:
- `app/lib/blog/post-detail/determineContentVisibility.ts`: サブスクリプション状態とfreeContentPercentageから可視範囲を判定する純粋関数

**シグネチャ**: `(hasActiveSubscription: boolean, freeContentPercentage: number) => { showFullContent: boolean, visiblePercentage: number }`

### 🚧data-io.test

**新規ファイル**:
- `app/data-io/blog/post-detail/getSubscriptionStatus.server.test.ts`: サブスクリプション状態取得のテスト（accountサービスdata-io層のモック使用）

**テストケース**:
- アクティブなサブスクリプション → `hasActiveSubscription: true`
- 非アクティブ/存在しない → `hasActiveSubscription: false`
- accountサービスdata-io層エラー → 安全側に倒す（`false`）

### 🪨data-io

**新規ファイル**:
- `app/data-io/blog/post-detail/getSubscriptionStatus.server.ts`: accountサービスのdata-io層を呼び出してサブスクリプション状態を取得

**重要な依存関係**:
- `app/data-io/account/subscription/getSubscriptionByUserId.server.ts`（accountサービス）
- この関数が実装されていることが前提

**既存ファイル更新**:
- `app/data-io/blog/post-detail/fetchPostBySlug.server.ts`: frontmatterから`freeContentPercentage`フィールドを読み込む処理追加

### その他

**重要な実装順序の考慮事項**:

1. **accountサービスの先行実装が必須**:
   - `app/data-io/account/subscription/getSubscriptionByUserId.server.ts`
   - `app/data-io/account/common/getSessionBySessionId.server.ts`
   - これらが実装されていない場合、blogサービスのアクセス制御機能はモックでのテストのみ可能

2. **サービス間連携のテスト戦略**:
   - 単体テスト: accountサービスdata-io層をモック化
   - 統合テスト: 実際のaccountサービス実装を使用
   - E2Eテスト: 完全なサービス間連携を検証

3. **段階的な実装アプローチ**:
   - Phase 1: lib層（`determineContentVisibility.ts`）の実装とテスト（accountサービス不要）
   - Phase 2: data-io層（`getSubscriptionStatus.server.ts`）のモックベース実装とテスト
   - Phase 3: UI層（Paywall、SubscriptionPromotionBanner）の実装とテスト
   - Phase 4: accountサービス実装後、統合テストとE2Eテスト

4. **フォールバック戦略**:
   - accountサービス未実装時: すべてのユーザーを未契約として扱う（安全側に倒す）
   - エラー時: ペイウォールを表示し、記事を制限する方向で動作

---

**次のアクション**: accountサービスのTDD_WORK_FLOW.mdで`getSubscriptionByUserId.server.ts`の実装順序を確認し、blogサービスの実装タイミングを調整する必要があります。
