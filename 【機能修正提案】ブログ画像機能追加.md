...
画像読み込み時のレイアウトシフト（CLS: Cumulative Layout Shift）を防止するため、以下を実装します。

| 項目 | 実装内容 |
|------|----------|
| **遅延読み込み** | `loading="lazy"` 属性を付与 |
| **アスペクト比固定** | CSS `aspect-ratio: 1200 / 630` を指定 |
| **コンテナサイズ予約** | 画像読み込み前からスペースを確保 |
...
```html
<img
  src="https://assets.claudemix.dev/blog/{slug}/thumbnail.webp"
  alt="記事サムネイル"
  loading="lazy"
/>
```
...
- CLS対策: `aspect-ratio: 1200 / 630`、`loading="lazy"` 指定
- サムネイル未存在時の表示仕様
...
- `app/styles/layer2-sections/blog.css` - サムネイル画像のスタイル追加
  - `.thumbnail-container` に `aspect-ratio: 1200 / 630` を設定（CLS対策）
  - 画像コンテナのレスポンシブ対応
